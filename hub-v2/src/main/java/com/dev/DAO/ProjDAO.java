...
//	"SELECT IS_MGR from \"RB_SCH_REAL\".\"rbei.real.i1.data.master.attributes::RA_DDL_ATTRIBUTES.T_RA_EMPL_M\" WHERE EMPNO = ?"
	...
	String projecteditstmt="UPDATE \"RB_SCH_REAL\".\"rbei.real.i1.data.master.attributes::RA_DDL_ATTRIBUTES.T_RA_PRJK\" SET START_DATE =?,END_DATE=?, CLIENT_ID = ?,POS_REQ = ?, MRG_NO = ?, PRJ_COST = ?,PRJ_STATUS=?,PRJ_DES=?,PRJ_LOC=? WHERE PRJ_NO =? ";
	/*----------------------------------------------------*/
	/*----------------------------------------------------*/
	                           ...
	                           if (IS_MGR.equals("3")){
	                        	   try
	                                  ...
	                                  if(sdate.before(edate))
	                                  {
	                                	  ...
	                                	  //START_DATE =?,END_DATE=?, CLIENT_ID = ?,POS_REQ = ?, MRG_NO = ?, PRJ_COST = ?,PRJ_STATUS=?,PRJ_DES=?
	                                  
	                                	     //pstmt.setString(1, proj.getProjectNumber());
	                                         //pstmt.setString(1, ProjectId);
											 pstmt.setString(3, proj.getClientId());//CLIENT_ID
											 pstmt.setString(1, proj.getStartDate());//START_DATE
	                                         pstmt.setString(2, proj.getEndDate());//END_DATE	
	                                         pstmt.setString(4, proj.getPosRequest());//POS_REQ
	                                         pstmt.setString(5,proj.getMrgNumber());//MRG_NO  
	                                         pstmt.setString(6,proj.getProjectcost());//PRJ_COST int
	                                         //
	                                         pstmt.setString(7,proj.getProjectStatus());//PRJ_STATUS 
	                                         pstmt.setString(8,proj.getProjectDes());//PRJ_DES
	                                         pstmt.setString(9,proj.getProjectlocation());//PRJ_LOC
	                                         pstmt.setString(10, proj.getProjectNumber());
                                             int i = pstmt.executeUpdate();
